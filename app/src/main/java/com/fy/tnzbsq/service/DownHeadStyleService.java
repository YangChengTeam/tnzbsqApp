...
public class DownHeadStyleService extends Service {
	...
	private void createNotification() {
		...
		notification = new Notification(R.drawable.logo, "开始下载个性头像", System.currentTimeMillis());
		...
		rv = new RemoteViews(this.getPackageName(), R.layout.headstyle_notification_layout);
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if(intent.getStringExtra("downUrl") != null && intent.getStringExtra("downUrl").length() >0){
			...
			handler.post(new Runnable() {
				@Override
				public void run() {
					Toast.makeText(DownHeadStyleService.this, "下载地址错误，请稍后重试",Toast.LENGTH_SHORT).show();
				}
			});
		}
		return 1;
	...
	private class DownLoadThread extends Thread {
		...
		public void run() {
			...
			downloadFile = new DownloadFile(url, Contants.HEAD_STYLE_SOURCE_FILE_PATH);
			...
			if (downloadFile.downloadFile()) {
				...
				boolean result = FileUtils.renameFile(Contants.HEAD_STYLE_SOURCE_FILE_PATH,Contants.HEAD_STYLE_DOWN_FILE_PATH);
				if(result){
					Intent intent = new Intent(Intent.ACTION_VIEW);
					intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
					intent.setDataAndType(Uri.fromFile(new File(Contants.HEAD_STYLE_DOWN_FILE_PATH)),
