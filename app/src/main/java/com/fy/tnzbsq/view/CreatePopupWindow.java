...
public class CreatePopupWindow extends PopupWindow {
    ...
    public CreatePopupWindow(Context context, OnClickListener itemsOnClick) {
        ...
        fromXDeltaLeft = (DensityUtils.getScreenW(context) - SizeUtils.dp2px(context,30)) / 2 - DensityUtils.dip2px(context, 150) / 2;
        fromXDeltaRight = -((DensityUtils.getScreenW(context) - SizeUtils.dp2px(context,30)) / 2 - DensityUtils.dip2px(context, 150) / 2);

        fromYDelta = 440 - create_image_layout.getHeight();
        int toYDelta = 440 - create_image_layout.getHeight() - 150;
        ...
        translateAnimationLeft.setInterpolator(new DecelerateInterpolator());
        translateAnimationRight.setInterpolator(new DecelerateInterpolator());
        translateAnimationLeft.setDuration(400);
        translateAnimationRight.setDuration(400);

        create_image_layout.startAnimation(translateAnimationLeft);
        create_word_layout.postDelayed(new Runnable() {
            @Override
            public void run() {
