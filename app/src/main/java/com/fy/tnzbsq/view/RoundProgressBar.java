...
public class RoundProgressBar extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		int temp = 6 - (progress/20);
		float textWidth = paint.measureText(temp + "s");   //测量字体宽度，我们需要根据字体的宽度设置在圆环中间
		
		if(textIsDisplayable && percent != 0 && style == STROKE){
			canvas.drawText("跳过", centre - textWidth + 5, centre, paint);
			canvas.drawText(temp + "s", centre - textWidth / 2, centre + textSize, paint); //画出进度百分比
		}
